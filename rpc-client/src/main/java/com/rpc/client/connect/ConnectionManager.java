...
import com.rpc.client.handler.RpcClientInitializer;
import com.rpc.client.route.RpcLoadBalance;
import com.rpc.client.route.impl.RpcLoadBalanceRoundRobin;
...
public class ConnectionManager {
    ...
    public void updateConnectedServer(List<RpcProtocol> serviceList) {
        // Now using 2 collections to manage the service info and TCP connections because making the connection is asyne
    ...
    public void updateConnectedServer(RpcProtocol rpcProtocol, PathChildrenCacheEvent.Type type) {
        ...
        if (type == PathChildrenCacheEvent.Type.CHILD_ADDED && !rpcProtocolSet.contains(rpcProtocol)) {
            ...
            // TODO We may don not need to reconnect remote server if server'IP and server's port are not changed
            removeAndCloseHandler(rpcProtocol);
            connectServerNode(rpcProtocol);
        }
